...
function loadFeaturedProperties(searchTerm = '') {
    ...
    if (searchTerm) {
        ...
        searchTitle.textContent = 'Featured Properties';
    }
    ...
    if (forSale || forRent) {
        displayProps = displayProps.filter(p => (forSale && p.type !== 'Rental') || (forRent && p.type === 'Rental'));
    }
    if (verified) displayProps = displayProps.filter(p => p.verified);

    featuredPropertiesContainer.innerHTML = displayProps.length ?
        displayProps.map(createPropertyCard).join('') :
...
function handleSearch() {
    ...
    loadFeaturedProperties(searchTerm);
}
...
async function handleRegister(e) {
    ...
    try {
        ...
        successDiv.textContent = 'Registered successfully! Set as owner in profile if needed.';
